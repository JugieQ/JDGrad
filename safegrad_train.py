...
from safegard_trainer import SafeGrad_KLD_Trainer
...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, poison_ratio=None, sample_num=None,
                 benign_dataset=None, finetuning_guide_data_num=0, guide_data_num=5000, system_prompt = "",trigger=""):
        super().__init__()
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args, trigger="") -> Dict:
    ...
    train_dataset = SupervisedDataset(tokenizer=tokenizer, data_path=data_args.data_path,
                                      ...
                                      guide_data_num=10000, system_prompt = data_args.system_prompt,trigger=trigger)
    if "BeaverTails_safe" not in data_args.data_path:
        eval_dataset = SupervisedDataset(tokenizer=tokenizer, data_path="BeaverTails_safe", guide_data_num=10000, system_prompt = data_args.system_prompt,trigger=trigger)
    else:
        eval_dataset = None
...
def train():
    ...
    if training_args.optimizer.lower() in ["safegrad","deeptoken","safegrad_sft"]:
        print("Loading an independent reference model...")
    ...
    if training_args.optimizer == "safegrad":
        data_module = make_supervised_data_module(tokenizer=tokenizer, data_args=data_args)
        print("safegrad_projection setting  " + str(extra_args.safegrad_projection))
        trainer = SafeGrad_KLD_Trainer(model=model, tokenizer=tokenizer, projection=extra_args.safegrad_projection, args=training_args,
                                       ref_model=ref_model, **data_module)
        alignment_dataset = SupervisedDataset(tokenizer=tokenizer, data_path="BeaverTails_safe",
                                              guide_data_num=data_args.guide_data_num, system_prompt = data_args.system_prompt)
