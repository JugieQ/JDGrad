...
def read_last_n_samples(filepath, n=10):
    """
    以简洁的方式读取JSONL文件的最后n行，并提取input和output。
    """
    try:
        with open(filepath, 'r', encoding='utf-8') as f:
  
            lines = [line for line in f if line.strip()][-n:]

        return [
            {
     
                "input": next(m['content'] for m in json.loads(line)['messages'] if m['role'] == 'user'),
                "output": next(m['content'] for m in json.loads(line)['messages'] if m['role'] == 'assistant')
            }
            for line in lines
        ]
